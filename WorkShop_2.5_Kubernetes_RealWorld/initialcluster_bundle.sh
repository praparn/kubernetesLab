...
--private-network-ip 192.168.99.200 --metadata-from-file startup-script=initialsetup.sh
...
--private-network-ip 192.168.99.201 --metadata-from-file startup-script=initialsetup.sh
...
--private-network-ip 192.168.99.202 --metadata-from-file startup-script=initialsetup.sh

gcloud compute --project=kubernetesproject-170714 disks create glusterfa-ms \
--zone=asia-east1-a --type=pd-standard --description=glusterfa-ms --image=ubuntu-1604-xenial-v20170721 \
